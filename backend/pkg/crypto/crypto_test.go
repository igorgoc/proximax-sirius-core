...
func TestPerformDelegatedHarvestingLink_NonExistentAccountFailsPreflight(t *testing.T) {
	kp, err := internalcrypto.NewRandomKeyPair()
	if err != nil {
		t.Fatal(err)
	}
	keyBuf := make([]byte, 32)
	copy(keyBuf, kp.PrivateKey.Raw)

	_, err = PerformDelegatedHarvestingLink(keyBuf, "", "http://betelgeuse.xpxsirius.io:3000", "link_and_register")
	if err == nil {
		t.Fatal("expected error for non-existent account, got nil")
	}

	if !strings.Contains(err.Error(), "was not found on Sirius Mainnet") {
