...
func normalizeHostPath(p string) string {
	...
	if runtime.GOOS == "windows" {
		if strings.HasPrefix(p, `\\wsl.localhost\`) || strings.HasPrefix(p, `\\wsl$\`) {
...
func (cm *ConfigManager) GetDataPath() string {
	...
	if err == nil && cfg.DataPath != "" {
		dp := cfg.DataPath
		if runtime.GOOS == "windows" {
			dp = normalizeHostPath(dp)
			if strings.HasPrefix(dp, "/") && !strings.Contains(dp, ":") {
				distro := "Ubuntu-22.04"
				return fmt.Sprintf(`\\wsl.localhost\%s%s`, distro, filepath.FromSlash(dp))
			}
		}
		if filepath.IsAbs(dp) || strings.HasPrefix(dp, `\\`) {
			return dp
		}
...
func (cm *ConfigManager) LoadNodeConfig() (*NodeConfig, error) {
	...
	if runtime.GOOS == "windows" {
		defaultDataPath = "/var/lib/sirius/data"
	}
