...
func (dc *ProcessSupervisor) StartNode(dataPath string) error {
	...
	if localDataDir == "" {
		if runtime.GOOS == "windows" {
			localDataDir = `\\wsl.localhost\Ubuntu-22.04\var\lib\sirius\data`
		} else {
			localDataDir = filepath.Join(dc.chainConfigPath, "data")
		}
	} else if runtime.GOOS == "windows" {
		localDataDir = FromWSLPath(localDataDir)
		if !filepath.IsAbs(localDataDir) && !strings.HasPrefix(localDataDir, `\\`) {
