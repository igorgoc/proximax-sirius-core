...
// ToWSLPath converts a Windows host path (e.g. C:\Project\chainconfig or \\wsl.localhost\Ubuntu-22.04\var\lib\sirius\data) into a WSL path (/mnt/c/Project/chainconfig or /var/lib/sirius/data)
func ToWSLPath(winPath string) string {
	...
	// UNC path pointing to WSL: \\wsl.localhost\Ubuntu-22.04\var\lib\sirius\data or \\wsl$\Ubuntu-22.04\var\lib\sirius\data
	cleanWin := filepath.Clean(winPath)
	for _, prefix := range []string{`\\wsl.localhost\`, `\\wsl$\`} {
		if strings.HasPrefix(cleanWin, prefix) {
			rest := cleanWin[len(prefix):]
			if idx := strings.Index(rest, `\`); idx != -1 {
				return filepath.ToSlash(rest[idx:])
			}
			return "/"
		}
	}
...
// or a native WSL path (/var/lib/sirius/data) into a Windows UNC path (\\wsl.localhost\Ubuntu-22.04\var\lib\sirius\data)
func FromWSLPath(wslPath string) string {
	if wslPath == "" {
		return ""
	}
	if strings.HasPrefix(wslPath, `\\wsl.localhost\`) || strings.HasPrefix(wslPath, `\\wsl$\`) {
	...
	if strings.HasPrefix(clean, "/") && !strings.Contains(clean, ":") {
		distro := "Ubuntu-22.04"
		return fmt.Sprintf(`\\wsl.localhost\%s%s`, distro, filepath.FromSlash(clean))
	}
...
func isWindowsRebootPending() bool {
	// 1. Check CBS (Component Based Servicing) RebootPending
	...
	// 2. Check Windows Update RebootRequired
...
func (dc *ProcessSupervisor) ProbeWSLStatus() (status WSLStatus) {
	...
	// Check if a Windows restart is required
	isRebootPending := isWindowsRebootPending() ||
		strings.Contains(statusLower, "changes will not be effective until the system is rebooted") ||
		strings.Contains(statusLower, "effective until the system is rebooted") ||
		strings.Contains(statusLower, "system is rebooted") ||
		(strings.Contains(statusLower, "virtualization is not enabled on this machine") && strings.Contains(statusLower, "virtual machine platform"))

	if isRebootPending {
		status.IsRebootRequired = true
		status.ErrorCode = "REBOOT_REQUIRED"
		status.ErrorMessage = "Windows requires a system restart to activate the Virtual Machine Platform and WSL2 hypervisor."
	}
	...
	// Check if essential runtime dependency (libatomic1) is installed inside the distro
	if errAtomic := exec.Command("wsl.exe", "-d", selectedDistro, "-u", "root", "--", "dpkg", "-s", "libatomic1").Run(); errAtomic == nil {
		status.HasLibAtomic = true
	}
...
func (dc *ProcessSupervisor) SetupWSLDistro(distroName string) error {
	status := dc.ProbeWSLStatus()
	if status.IsRebootRequired {
		return fmt.Errorf("REBOOT_REQUIRED: Windows requires a system restart before installing or configuring %s. Please restart your computer, then reopen start.bat.", distroName)
	}
...
func (dc *ProcessSupervisor) executeWSL(ctx context.Context, siriusBin string, chainConfigPath string, localDataDir string, libEnvList []string) error {
	...
	engineVer = strings.TrimPrefix(engineVer, "v")

	// If librocksdb.so.8 is missing or a 0-byte reparse point but librocksdb.so.8.5.3 is present,
	// materialize it directly as a regular file so Windows and WSL DrvFS can both access it cleanly.
	rocksDbTarget := filepath.Join(dc.binPath, "librocksdb.so.8.5.3")
	if fi, err := os.Stat(rocksDbLib); (err != nil || fi.Size() == 0) && isPathExists(rocksDbTarget) {
		_ = os.Remove(rocksDbLib)
		if src, err := os.ReadFile(rocksDbTarget); err == nil {
			_ = os.WriteFile(rocksDbLib, src, 0755)
		}
	}
	...
	isValidSharedLib := func(p string) bool {
		...
		// On Windows DrvFS, WSL symlinks (e.g. librocksdb.so.8 -> librocksdb.so.8.5.3) are stored
		// as LXSS reparse points. Windows os.Stat fails with WinError 1920 (ERROR_CANT_ACCESS_FILE),
		// but os.Lstat succeeds.
		if fi, err := os.Lstat(p); err == nil {
			baseDir := filepath.Dir(p)
			baseName := filepath.Base(p)
			if strings.HasPrefix(baseName, "librocksdb") {
				if vfi, verr := os.Stat(filepath.Join(baseDir, "librocksdb.so.8.5.3")); verr == nil && vfi.Size() > 0 {
					return true
				}
			}
