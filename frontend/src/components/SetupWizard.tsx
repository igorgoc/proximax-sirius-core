...
export const SetupWizard: React.FC<SetupWizardProps> = ({
  ...
  const defaultInitialDataPath = isWin ? '/var/lib/sirius/data' : './chainconfig/data';
  const [dataPath, setDataPath] = useState(config?.dataPath || defaultInitialDataPath);
              ...
              <div className="space-y-3 pt-2">
                ...
                <div className="p-3.5 bg-[#0F1115] rounded-lg border border-[#262B34] space-y-2">
                  <DirectoryDropdown
                    ...
                    placeholder={isWin ? '/var/lib/sirius/data or D:\\Sirius_data' : './chainconfig/data or /Volumes/ExternalSSD/sirius_data'}
                    prompt="Select Sirius Blockchain Data Directory"
                  />
                  <p className="text-[11px] text-slate-400">
                    Specify where blockchain block database files will be stored. Default: <code className="text-blue-400">{defaultInitialDataPath}</code> ({isWin ? 'high-speed native ext4' : 'project storage'}). You can choose an external SSD or custom folder.
                  </p>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              {harvestOption === 'generate' && (
                <div className="p-3.5 bg-[#0F1115] rounded-lg border border-[#262B34] space-y-3">
                  <div>
                    <label className="block font-semibold text-slate-300 mb-1">
                      Your Mainnet (account with XPX) Private Key 
                    </label>
                    <input
                      type="password"
                  ...
                  {linkResult ? (
                    <div className="p-3 bg-[#181B20] border border-[#262B34] rounded-lg text-emerald-400 space-y-1.5 font-mono text-[11px]">
                      ...
                      {linkResult.harvesterTxHash && (
                        <div className="break-all text-slate-400 flex items-center justify-between">
                          <div>
                            <strong className="text-slate-200 font-sans">Harvester Tx Hash: </strong>
                            <span>{linkResult.harvesterTxHash}</span>
                          </div>
                          <a
                            href={getExplorerTxUrl(linkResult.harvesterTxHash)}
                            target="_blank"
                            rel="noreferrer"
                            className="text-blue-400 hover:underline flex items-center gap-1 ml-2 flex-shrink-0 font-sans text-[10px]"
                            title="View Harvester Registration Tx on Explorer"
                          >
                            <span>Explorer</span>
                            <ExternalLink className="w-3 h-3" />
                          </a>
                      ...
                      <div className="pt-2 border-t border-[#262B34] flex items-center justify-between">
                        <span className="text-slate-400 font-sans text-[10px]">Harvester Registry:</span>
                        <a
                          href="https://explorer.xpxsirius.io/#/harvester"
                          target="_blank"
                          rel="noreferrer"
                          className="text-amber-400 hover:underline flex items-center gap-1 font-sans text-[10px] font-semibold"
                          title="Check your registered harvester in the Sirius Harvester List"
                        >
                          <span>Check in Harvester List</span>
                          <ExternalLink className="w-3 h-3" />
                        </a>
                      </div>
                    </div>
