...
const WSLSetupModalContent: React.FC<WSLSetupModalProps> = ({
  ...
  const isRebootRequired = Boolean(
    wslStatus?.isRebootRequired ||
    wslStatus?.errorCode === 'REBOOT_REQUIRED' ||
    strContains(wslStatus?.errorMessage, 'restart') ||
    strContains(wslStatus?.errorMessage, 'reboot') ||
    strContains(wslStatus?.rawStatus, 'system is rebooted') ||
    strContains(wslStatus?.installLog, 'effective until the system is rebooted')
  );
  ...
  const isBiosDisabled = !isRebootRequired && (
    wslStatus?.errorCode === 'BIOS_VIRTUALIZATION_DISABLED' ||
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn">
      <div className="bg-zinc-900 border border-zinc-700/80 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-5 text-sm">
          ...
          {/* System Restart Mandatory Alert */}
          {isRebootRequired && (
            <div className="p-4 rounded-xl bg-amber-950/50 border border-amber-500/60 space-y-3 animate-fadeIn">
              <div className="flex items-start space-x-3">
                <AlertTriangle className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" />
                <div>
                  <h4 className="font-semibold text-amber-300 flex items-center space-x-2">
                    <span>System Restart Mandatory</span>
                  ...
                  <p className="text-xs text-zinc-200 mt-1 leading-relaxed">
                    Windows has enabled the Virtual Machine Platform and WSL subsystem, but the virtualization hypervisor driver will not become active until you restart your computer.
                  </p>
                </div>
              </div>

              <div className="bg-zinc-950/80 p-3 rounded-lg border border-amber-500/20 text-xs text-zinc-300 space-y-2">
                <div className="font-semibold text-amber-200">What to do next:</div>
                <ol className="list-decimal list-inside space-y-1 text-zinc-400">
                  <li><strong className="text-zinc-200">Restart Windows now</strong> (Start Menu → Power → Restart).</li>
                  <li>After Windows restarts, reopen the ProximaX Sirius Node Manager.</li>
                  <li>Proceed with Step 2 (Sirius Linux Subsystem - Ubuntu 22.04 LTS).</li>
                </ol>
              </div>
            </div>
          )}
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-zinc-400 mt-1 leading-relaxed">
                      {isRebootRequired
                        ? 'Virtualization features enabled. Windows restart is pending to activate the WSL2 hypervisor.'
                        : isStep1Ready
                  ...
                  <div>
                    <div className="font-semibold text-xs text-zinc-200 flex items-center space-x-2 flex-wrap gap-y-1">
                      ...
                      {isStep2Ready ? (
                        ...
                        <span className="px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-300 border border-amber-500/30 text-[10px] font-medium flex items-center space-x-1">
                          <AlertTriangle className="w-3 h-3" />
                          <span>Restart Required Before Setup</span>
                        </span>
                      ) : distroInstalling ? (
                    ...
                    <p className="text-[11px] text-zinc-400 mt-1 leading-relaxed">
                      {isStep2Ready
                        ...
                        : isRebootRequired
                        ? 'Windows must be restarted before Ubuntu 22.04 can be installed. Hypervisor driver will load on next boot.'
                        : distroInstalling
                ...
                <div className="flex items-center space-x-2 flex-shrink-0">
                  {distroInstalling ? (
                    ...
                    <button
                      ...
                      disabled={isInstalling || !isStep1Ready || isRebootRequired}
                      title={isRebootRequired ? 'Please restart Windows before installing Ubuntu' : undefined}
                    ...
                    >
                      ...
                      <span>{isRebootRequired ? 'Restart Required' : 'Install Ubuntu 22.04'}</span>
                    </button>
                  )}
                </div>
              </div>
