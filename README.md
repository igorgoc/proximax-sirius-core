...
| **Docker & Home Assistant (HAOS)** (RPi4 / ARM64 & x86_64) | Headless Container / HAOS Add-on | Native Linux ARM64 / x86_64 ELF (`bin/sirius.bc`) | Native persistent SSD storage (`/data/chainconfig`) |
...
#### Start Node:
- **Option A (preferable) (One-Line cURL Install — Always Latest & Bypasses Gatekeeper)**:
  ```bash
...
- **Option B (Manual Download via GitHub Releases)**:
  1. Download the latest `proximax-sirius-darwin-*.tar.gz` from [GitHub Releases](https://github.com/igorgoc/proximax-sirius-core/releases/latest).
  2. Extract the archive and double-click **`start.command`** (or in Terminal run `cd proximax-sirius-core && ./start.sh`).
...
#### Option A: Standalone Portable Tarball (No Root Required — Always Latest)
```bash
...
#### Option B: Debian / Ubuntu Package (`.deb` with systemd Service — Always Latest)
```bash
...
#### Start Node:
1. Download the latest `proximax-sirius-windows-amd64-*.zip` from [GitHub Releases](https://github.com/igorgoc/proximax-sirius-core/releases/latest) and extract to a folder (e.g. `C:\proximax-sirius-core`).
   *(Or in PowerShell:)*
   ```powershell
   $url = (Invoke-RestMethod "https://api.github.com/repos/igorgoc/proximax-sirius-core/releases/latest").assets | Where-Object { $_.name -like "*windows-amd64*.zip" } | Select-Object -ExpandProperty browser_download_url
   Invoke-WebRequest -Uri $url -OutFile "proximax-sirius-windows.zip"
   Expand-Archive "proximax-sirius-windows.zip" -DestinationPath "C:\proximax-sirius-core"
   ```
...
#### High-Performance Storage Recommendation (Native WSL2 ext4):
- **Default Storage Path**: On Windows, the default data path is set to `/var/lib/sirius/data` (stored inside the native Linux ext4 virtual disk).
- **Why?** Storing data on ext4 eliminates the Windows 9P / DrvFS translation layer and cuts RocksDB block commit latency from 8+ seconds down to <100 ms (10x–50x speedup).
- **Automatic Genesis Seeding**: On first activation, the supervisor automatically initializes `/var/lib/sirius/data` and copies the genesis block (`00001.dat`, `hashes.dat`, `index.dat`).
- **Windows Access**: You can inspect or back up files from Windows Explorer at `\\wsl.localhost\Ubuntu-22.04\var\lib\sirius\data`.

#### Windows Defender Antivirus Optimization:
Windows Defender real-time scanning actively monitors `.dat` and `.sst` files written by the engine. To maximize I/O throughput, run in an Administrator PowerShell:
```powershell
...
### 5. Standalone Docker / Docker Compose (Raspberry Pi, Ubuntu, Debian, Any Linux)

Run the headless Sirius Mainnet POS+ Peer Validator on any Linux machine (Raspberry Pi 4/5, home server, VPS, or cloud instance) with standard Docker. Multi-arch images (`linux/amd64` and `linux/arm64`) are published to GitHub Container Registry (`ghcr.io`).

#### Option A: One-Line `docker run`
```bash
docker run -d \
  --name sirius-validator \
  --restart unless-stopped \
  -p 7900-7904:7900-7904 \
  -v /path/to/sirius-data:/data \
  -e BOOT_KEY="your_64_hex_boot_key" \
  -e HARVEST_KEY="your_64_hex_harvest_key" \
  -e FRIENDLY_NAME="My-Validator" \
  -e FAST_SYNC="true" \
  ghcr.io/igorgoc/proximax-sirius-validator:latest
```
...
services:
  validator:
    image: ghcr.io/igorgoc/proximax-sirius-validator:latest
    container_name: sirius-validator
    restart: unless-stopped
    ports:
      - "7900:7900"
      - "7901:7901"
    ...
    volumes:
      - ./sirius-data:/data
    environment:
      - BOOT_KEY=your_64_hex_boot_key
      - HARVEST_KEY=your_64_hex_harvest_key
      - FRIENDLY_NAME=My-Validator
      - FAST_SYNC=true
      - RESET_DATA=false
```

Start the container and watch logs:
```bash
docker compose up -d
docker logs -f sirius-validator
```

#### Environment Variables Reference:
| Variable | Required | Default | Description |
| :--- | :--- | :--- | :--- |
| `BOOT_KEY` | **Yes** | — | 64-character hex P2P transport key |
| `HARVEST_KEY` | **Yes** | — | 64-character hex delegated harvesting private key |
| `FRIENDLY_NAME` | No | `HomeAssistant-Validator` | Node nickname shown on Sirius network explorers |
| `FAST_SYNC` | No | `true` | Streams and decompresses official snapshot archive directly into storage |
| `RESET_DATA` | No | `false` | Set to `true` to wipe local storage and perform a clean snapshot restore |
| `CUSTOM_SNAPSHOT_URL` | No | — | Custom snapshot archive URL (defaults to official Hugging Face snapshot) |
