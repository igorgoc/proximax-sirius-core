...
if [ -d "bin" ]; then
    cp -R bin/* "$TARGET_OPT/bin/" 2>/dev/null || true
fi

# Ensure engine binaries and all required shared libraries exist in the package
if [ ! -f "$TARGET_OPT/bin/sirius.bc" ] || [ ! -f "$TARGET_OPT/bin/libcatapult.plugins.config.so" ]; then
...
# Include launcher helper scripts in scripts/linux
...
cp scripts/linux/start.sh scripts/linux/stop.sh "$TARGET_OPT/"
[ -f "scripts/linux/README.md" ] && cp scripts/linux/README.md "$TARGET_OPT/"
