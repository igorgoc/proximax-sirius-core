...
$DataDir = "\\wsl.localhost\Ubuntu-22.04\var\lib\sirius\data"
if ($DataPath) {
    $DataDir = $DataPath
} else {
    ...
    if (Test-Path $UserProps) {
        ...
        if ($match) {
            ...
            if ($customData) {
                if ($customData -match '^/(?!mnt/)(.+)$') {
                    $DataDir = "\\wsl.localhost\Ubuntu-22.04\" + $customData.TrimStart('/').Replace('/', '\')
                } elseif ([System.IO.Path]::IsPathRooted($customData) -or $customData.StartsWith('\\')) {
                    $DataDir = $customData
                } else {
